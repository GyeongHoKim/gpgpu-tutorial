...
// (14·18장 shaders/bilinear-upscale.wgsl 을 그대로 가져왔다.)
//
// 19장(FSRCNN)에서 이 셰이더는 SRCNN 처럼 "사전 확대"로 쓰이지 않는다. FSRCNN 은
// LR 해상도에서 conv 를 다 끝내고 마지막 deconvolution 으로 확대하기 때문이다.
// 여기서는 오직 "고정 방식(bilinear) 확대" vs "학습된 deconvolution 확대" 를 나란히
// 비교하기 위한 baseline 으로만 쓴다.
//
// 좌표 규약(14장과 동일):
